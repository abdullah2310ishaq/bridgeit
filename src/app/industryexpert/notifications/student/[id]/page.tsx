  ...
  useEffect(() => {
    const fetchStudentDetails = async () => {
      ...
      try {
        ...
        let response = await fetch(`https://localhost:7053/api/get-student/student-by-id/${id}`, {
          headers: { Authorization: `Bearer ${token}` },
        })

        // If that fails, try by studentId
        if (!response.ok) {
          response = await fetch(`https://localhost:7053/api/get-student/student-by-student-id/${id}`, {
            headers: { Authorization: `Bearer ${token}` },
          })
        }
