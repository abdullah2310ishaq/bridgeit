...
export default function PurchasedProjectsPage() {
  ...
  useEffect(() => {
    const fetchPurchasedProjects = async () => {
      ...
      try {
        ...
        const boughtFypsResponse = await fetch("https://localhost:7053/api/bought-fyp/get-all", {
          headers: { Authorization: `Bearer ${token}` },
        })
        ...
        for (const id of boughtFypIds) {
          ...
          const boughtFypResponse = await fetch(`https://localhost:7053/api/bought-fyp/by-id/${id}`, {
            headers: { Authorization: `Bearer ${token}` },
          })
          ...
          if (boughtFypResponse.ok) {
            ...
            if (boughtFypData.indExpertId === expertData.indExptId) {
              ...
              const fypResponse = await fetch(`https://localhost:7053/api/fyp/get-detailed-fyp-by-id/${boughtFypData.fypId}`, {
                headers: { Authorization: `Bearer ${token}` },
              })
  ...
  const handleDownloadAgreement = async (boughtFypId: string) => {
    ...
    try {
      ...
      const response = await fetch(`https://localhost:7053/api/bought-fyp/get-agreement/${boughtFypId}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
