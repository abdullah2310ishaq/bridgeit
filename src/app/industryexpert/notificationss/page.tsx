...
const NotificationsPage: React.FC = () => {
  ...
  const initiatePaymentForProposal = async (proposalId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        if (clientSecret) {
          console.log("Setting paymentClientSecret:", clientSecret); // Debugging line
  ...
  const handlePaymentSuccess = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${BACKEND_URL}/api/project-proposals/confirm-accept-proposal/${selectedProposal.id}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  if (proposals.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 text-white">
        <div className="mb-6">
          {/* Uncomment and replace with your own image if available */}
          {/* <img
