...
const FypRequestsPage: React.FC = () => {
    ...
    const fetchRequests = async () => {
      try {
        ...
        const adminResponse = await fetch(
          `http://api-bridgeit-htb0fpcee0ajb7a2.westindia-01.azurewebsites.net/api/get-uni-admins/admins-by-id/${userId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        const requestsResponse = await fetch(
          `http://api-bridgeit-htb0fpcee0ajb7a2.westindia-01.azurewebsites.net/api/ind-expert-request-fyp/pending-for-admin/${universityId}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const handleApprove = async (requestId: string) => {
    ...
    try {
      const response = await fetch(
        `http://api-bridgeit-htb0fpcee0ajb7a2.westindia-01.azurewebsites.net/api/ind-expert-request-fyp/approve/${requestId}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleReject = async (requestId: string) => {
    ...
    try {
      const response = await fetch(
        `http://api-bridgeit-htb0fpcee0ajb7a2.westindia-01.azurewebsites.net/api/ind-expert-request-fyp/reject/${requestId}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
